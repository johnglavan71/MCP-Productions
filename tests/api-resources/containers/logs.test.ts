...
describe('resource logs', () => {
  ...
  test.skip('retrieve', async () => {
    const responsePromise = client.containers.logs.retrieve('id');
  ...
  test.skip('stream', async () => {
    const responsePromise = client.containers.logs.stream('id');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('stream: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.containers.logs.stream('id', { env: 'env', tail: 'tail' }, { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Test2w.NotFoundError);
  });
});
