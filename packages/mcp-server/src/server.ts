...
import { ClientOptions } from 'test2w';
...
async function getInstructions(stainlessApiKey: string | undefined): Promise<string> {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/test2w',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the test2w MCP server. You will use Code Mode to help the user perform
