...
import { APIPromise } from 'test2w/core/api-promise';

import util from 'node:util';
import Test2w from 'test2w';
import { APIUserAbortError } from 'test2w';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['TEST2W_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Test2w) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['TEST2W_LOG'] = 'debug';
      const client = new Test2w({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['TEST2W_LOG'] = 'not a log level';
      const client = new Test2w({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'TEST2W_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Test2w({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['TEST2W_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Test2w({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['TEST2W_BASE_URL'] = 'https://example.com/from_env';
      const client = new Test2w({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Test2w({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['TEST2W_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Test2w({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Test2w({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Test2w({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['TEST2W_API_KEY'] = 'My API Key';
    const client = new Test2w();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['TEST2W_API_KEY'] = 'another My API Key';
    const client = new Test2w({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Test2w({ apiKey: 'My API Key', fetch: testFetch });
